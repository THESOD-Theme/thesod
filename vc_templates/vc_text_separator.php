...
if ( '' !== $title && 'separator_no_text' !== $layout ) {
	...
	if($title_level == 'styled-subtitle') {
		$content .= '<div class="styled-subtitle">' . $title . '</div>';
	} elseif($title_level == 'xlarge') {
		$content .= '<div class="title-xlarge'.($title_light ? ' light' : '').'">' . $title . '</div>';
	} elseif(in_array($title_level, array('h1', 'h2', 'h3', 'h4', 'h5', 'h6'))) {
		$content .= '<'.$title_level.($title_light ? ' class="light"' : '').'>' . $title . '</'.$title_level.'>';
	} else {
		$content .= '<h4>' . $title . '</h4>';
	}
}
