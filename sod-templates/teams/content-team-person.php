...
<div class="<?php echo esc_attr($thesod_grid_class);?> inline-column">
	<div id="post-<?php the_ID(); ?>" <?php post_class(array('team-person', 'centered-box', 'default-background')); ?> style="<?php if($params['background_color']) : ?>background-color: <?php echo $params['background_color'] ?>;<?php endif; ?> <?php if($params['border_color']) : ?>border-color: <?php echo $params['border_color'] ?><?php endif; ?>">
		<?php if(has_post_thumbnail()) : ?><div class="team-person-image"><?php echo $thesod_image_start; thesod_post_thumbnail('thesod-person-240', false, 'img-responsive', array('srcset' => array('2x' => 'thesod-person'))); echo $thesod_image_end; ?></div><?php endif; ?>
		<div class="team-person-info">
			<?php echo thesod_get_data($thesod_item_data, 'name', '', '<div class="team-person-name title-h4" '.($params['name_color'] ? 'style="color: '.esc_attr($params['name_color']).'"' : '').'>', '</div>'); ?>
