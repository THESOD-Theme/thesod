(function($) {
	...
	LineDiagram.prototype = {
		...
		showLine: function(queue, first) {
			...
			function thesod_show_digram_line_animation() {
			...
			if (first) {
				thesod_show_digram_line_animation();
			} else {
				this.startTimer(thesod_show_digram_line_animation);
			}
		},
	...
	jQuery.fn.thesod_start_line_digram = function() {
		return new LineDiagram(this.get(0));
	}
})(jQuery);

function thesod_show_diagram_line_mobile($box) {
	jQuery('.skill-element', $box).each(function () {
		jQuery('.skill-line div', this).width(jQuery('.skill-line div', this).data('amount') + '%');
	});
}

function thesod_start_line_digram(element) {
	jQuery(element).thesod_start_line_digram();
}
...
jQuery('.digram-line-box').each(function () {
	...
	jQuery(document).ready(function() {
		if (!jQuery(self).hasClass('lazy-loading-item') || window.sodSettings.lasyDisabled)
			jQuery(self).thesod_start_line_digram();
	});
});
