...
<div class="<?php echo esc_attr($thesod_grid_class); ?> inline-column">
	<div id="post-<?php the_ID(); ?>" <?php post_class(array('team-person', 'bordered-box')); ?> style="<?php if($params['background_color']) : ?>background-color: <?php echo $params['background_color'] ?>;<?php endif; ?> <?php if($params['border_color']) : ?>border-color: <?php echo $params['border_color'] ?><?php endif; ?>">
		<div class="team-person-box clearfix<?php echo ($params['columns'] < 3 ? ' team-person-box-columns' : ''); ?>">
		<?php if(has_post_thumbnail()) : ?><div class="team-person-box-left"><div class="team-person-image"><?php echo $thesod_image_start; thesod_post_thumbnail('thesod-person-240', false, 'img-responsive', array('srcset' => array('2x' => 'thesod-person'))); echo $thesod_image_end; ?></div></div><?php endif; ?>
		<div class="team-person-box-right">
			<div class="team-person-info">
				<?php echo thesod_get_data($thesod_item_data, 'name', '', '<div class="team-person-name '.($params['columns'] == 1 ? 'title-h2' : 'title-h4').'" '.($params['name_color'] ? 'style="color: '.esc_attr($params['name_color']).'"' : '').'><span class="light">', '</span></div>'); ?>
				<?php echo thesod_get_data($thesod_item_data, 'position', '', '<div class="team-person-position date-color'.($params['columns'] == 1 ? ' styled-subtitle' : '').'" '.($params['position_color'] ? 'style="color: '.esc_attr($params['position_color']).'"' : '').'>', '</div>'); ?>
			<?php if(!empty($thesod_item_data['phone'])) : ?>
				<div class="sod-styled-color-1"><div class="team-person-phone <?php echo ($params['columns'] == 1 ? 'title-h3' : 'title-h5'); ?>"><span class="light"><a href="<?php echo esc_url('tel:'.$thesod_item_data['phone']); ?>"<?php echo ($params['tel_color'] ? ' style="color: '.esc_attr($params['tel_color']).'"' : ''); ?>><?php echo esc_html($thesod_item_data['phone']); ?></a></span></div></div>
			<?php endif; ?>
