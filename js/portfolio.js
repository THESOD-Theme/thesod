	...
	$(function() {
		...
		function portfolio_load_more_request($portfolio, $set, is_scroll) {
			...
			if (is_scroll) {
				...
				$('.portfolio-load-more .sod-button', $portfolio).before('<div class="loading"></div>');
			}
		...
		function portfolio_load_core_request($portfolio) {
			...
			$('.portfolio-load-more .sod-button', $portfolio).before('<div class="loading"><div class="preloader-spin"></div></div>');
			...
			$.ajax({
				...
				success: function(response) {
					if (response.status == 'success') {
						...
						alert(response.message);
						$('.portfolio-load-more .sod-button .loading', $portfolio).remove();
					}
				}
			});
		}
		...
		function initPortfolioGrid() {
			...
			portfolio_images_loaded($set, '.image-inner img', function() {
				...
				if (!window.sodSettings.lasyDisabled) {
				...
				if (isNewsGrid) {
					$($set).on('gallery-inited', '.sod-simple-gallery', function() {
						if ($set.data('isotope')) {
							$set.isotope('layout');
						}
					});
		...
		function initPortfolioSlider() {
			...
			portfolio_images_loaded($set, '.image-inner img', function() {
				init_circular_overlay($portfolio, $set);
				if ($portfolio.hasClass('sod-slider-animation-dynamic')) {
					$set.juraSlider({
						type: 'dynamic',
				...
				if ($portfolio.hasClass('sod-slider-animation-one')) {
					$set.juraSlider({
						type: 'one',
						duration: 500,
						element: '.portfolio-item',
