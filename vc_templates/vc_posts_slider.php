...
while ( $my_query->have_posts() ) {
	...
	if ( $slides_content == 'teaser' ) {
		$content = apply_filters( 'the_content', get_the_content() ); //get_the_excerpt();
	} else {
		$content = '';
	}
	$thumbnail = '';
	...
	if ( $link != 'link_no' ) {
		if ( $link == 'link_post' ) {
			$link_image_start = '<a class="link_image" href="' . esc_url(get_permalink( $post_id )) . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'thesod' ), the_title_attribute( 'echo=0' ) ) . '">';
			...
			$link_image_start = '<a class="link_image prettyphoto" href="' . esc_url($p_link) . '" title="' . the_title_attribute( 'echo=0' ) . '" >';
...
if ( $teasers ) {
	...
	$teasers = esc_html__( 'Nothing found.', 'thesod' );
}
